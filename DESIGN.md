...
项目主图在桌面端嵌入带色舞台，移动端舞台延伸到屏幕边缘，让小屏仍保留足够的界面可读面积。620px 以下简化导航，但所有内容仍可线性滚动访问。
...
- “更多产品”使用 Flowing Menu：默认保持项目名与说明可扫描，桌面悬停或键盘聚焦后整行翻入深色循环字幕与真实项目缩略图；触屏端退化为静态缩略图列表。
...
- 项目舞台以整块项目色幕揭开，截图同时从放大与模糊状态收束，并在滚动期间保留轻微纵向视差和旋转。
