...
项目主图在桌面端嵌入带色舞台，移动端舞台延伸到屏幕边缘，让小屏仍保留足够的界面可读面积。每个精选项目提供三张同项目界面预览，桌面悬浮、键盘聚焦与移动端点击使用同一组视图标签。620px 以下简化导航，但所有内容仍可线性滚动访问。
...
- 联系区使用以 `#A1B1BD` 为核心的 Color Bends WebGL 色带作为收尾场景，色带仅在区块可见且页面位于前台时运行；主标题使用单次 Text Type 逐字切入，完成后保留低频光标呼吸。
- 联系区右侧使用 Lanyard 身份牌作为收尾焦点：绳带会跟随本人照片卡片的拖拽发生弯曲，松手后弹簧回正；卡面只使用本人照片、个人 SVG Logo 与真实职业信息。键盘方向键也可移动卡片，Enter 或 Esc 回正。
...
- 项目舞台以整块项目色幕揭开，截图同时从放大与模糊状态收束，并在滚动期间保留轻微纵向视差和旋转；界面预览切换使用短促的裁切、轻微模糊与等比缩放，软件画面始终保持原始比例。
